...
node {
    docker.image('docker:27.2-dind')
        ...
        properties([
            parameters([
                ...
                choice(
                    ...
                    description: 'Select the microservice to build/push (or special option)'
                )
            ])
        ])
        ...
        if (selectedService == 'auto') {
            def detected = sh(script: "ls src/ | head -1 || echo 'adservice'", returnStdout: true).trim()
            selectedService = detected ?: 'adservice'
            echo "Auto-detected service: ${selectedService}"
        }

        if (selectedService == 'all') {
            servicesToBuild = SERVICE_CHOICES.findAll { it != 'all' && it != 'auto' && it != 'none' }
        } else if (selectedService == 'none') {
            servicesToBuild = []
        } else {
            servicesToBuild = [selectedService]
        }
        ...
        try {
                    ...
                    stage("Build ${service}") {
                        sh """
                            docker build \
                                ...
                                -t ${params.SERVICE_REPO}/${service}:latest \
                                src/${service}
                        """
                    }
                    ...
                    stage("Push ${service}") {
                        withCredentials([[
                            ...
                            sh """
                                ...
                                docker tag ${params.SERVICE_REPO}/${service}:latest \
                                           ${params.ECR_REGISTRY}/${params.SERVICE_REPO}/${service}:latest

                                docker push ${params.ECR_REGISTRY}/${params.SERVICE_REPO}/${service}:latest

                                docker logout ${params.ECR_REGISTRY} || true
                            """
                        }
                    }
                }
            ...
            stage('Publish Checkov Results') {
                recordIssues(
                    ...
                    tool: sarif(pattern: 'checkov-results/results_sarif.sarif', name: 'Checkov Security Scan', id: 'checkov'),
                    qualityGates: [
            ...
            stage('Publish Semgrep Results') {
                recordIssues(
                    enabledForFailure: true,
                    tool: sarif(pattern: '**/semgrep.sarif', name: 'Semgrep Security Scan', id: 'semgrep'),
                    qualityGates: [
            ...
            stage('Archive Reports') {
                archiveArtifacts artifacts: 'semgrep.sarif, leaks.json', allowEmptyArchive: true, fingerprint: true
            }
            ...
            stage('Cleanup') {
                if (servicesToBuild.size() > 0) {
                    def images = servicesToBuild.collect { service ->
                        "${params.SERVICE_REPO}/${service}:latest ${params.ECR_REGISTRY}/${params.SERVICE_REPO}/${service}:latest"
                    }.join(' ')
