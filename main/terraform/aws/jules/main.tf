...
################################################################################
# Cleanup Logic (CloudFormation Custom Resource)
################################################################################
...
def handler(event, context):
    ...
    if event['RequestType'] == 'Delete':
        try:
            print(f"Listing services in cluster {cluster_name}")
            paginator = ecs.get_paginator('list_services')
            for page in paginator.paginate(cluster=cluster_name):
                for service_arn in page['serviceArns']:
                    try:
                        print(f"Updating service {service_arn} to desiredCount=0")
                        ecs.update_service(
                        ...
                        print(f"Failed to update service {service_arn}: {e}")
                        # Continue to try other services even if one fails
        except Exception as e:
            print(f"Error: {e}")
            # We don't fail the stack deletion, otherwise Terraform gets stuck
            # status = 'FAILED'
...
resource "aws_lambda_function" "cleanup" {
  ...
  function_name    = "${var.project_name}-${terraform.workspace}-cleanup-lambda"
...
resource "aws_cloudformation_stack" "cleanup" {
  name = "${var.project_name}-${terraform.workspace}-cleanup-stack"

  template_body = <<EOF
{
  "Resources": {
    "CleanupTrigger": {
      "Type": "Custom::ECSScaleDown",
      "Properties": {
        "ServiceToken": "${aws_lambda_function.cleanup.arn}",
